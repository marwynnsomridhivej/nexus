...
class VCPayload(WrapperBase):
    ...
    def __str__(self) -> str:
        params = [
            f"vc_listen_id={self.__vc_listen_id}",
            f"vc_dest_id={self.__vc_dest_id}",
            f"player_ids={self.__player_ids}",
            f"stop={self.__stop}",
        ]
        return f"VCPayload[{", ".join(params)}]"
...
class MatchFinalisedPayload(WrapperBase):
    ...
    @classmethod
    def create(cls, guild_id: int, entry: MatchEntry) -> "MatchFinalisedPayload":
        return cls({
            "guild_id": guild_id,
            "lobby_vc_id": entry.voice_channel_id,
            "winning_team": entry.winning_team,
            "losing_team": entry.losing_team,
        })
